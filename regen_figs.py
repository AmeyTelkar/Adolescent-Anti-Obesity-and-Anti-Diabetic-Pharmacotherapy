...
Fixes applied vs. the previous versions:
  ...
  * Forest plots rebuilt from the CORRECTED S8 table (N >= 3 and CI lower > 1 only)
...
OUT = "e:/Adverse drugs Project 2/PLOS ONE Jornal/PLOS ONE COLOUR/Reduced journal colour/PONE_Revision_V2/25 pages short/PLOS_Submission_Package_V17updated/generated_output"
os.makedirs(OUT, exist_ok=True)

# ---------------------------------------------------------------- load S8
S8 = "e:/Adverse drugs Project 2/PLOS ONE Jornal/PLOS ONE COLOUR/Reduced journal colour/PONE_Revision_V2/25 pages short/PLOS_Submission_Package_V17updated/Supporting_Documents/S8_Table_Complete_Signal_Detection.docx"
rows = []
for r in Document(S8).tables[0].rows[1:]:
    c = [x.text.strip() for x in r.cells]
    rows.append(dict(drug=c[0], pt=c[1], n=int(c[2]), ror=float(c[3]),
                     lo=float(c[4]), hi=float(c[5]), sig=c[6]))
signals = [r for r in rows if r["sig"] == "YES"]
print(f"[data] S8 rows={len(rows)}  confirmed signals={len(signals)}")
